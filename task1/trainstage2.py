...
def train(args,summary):
    if torch.cuda.is_available() and config.use_gpu:
        DEVICE = torch.device("cuda:" + str(3))   #config.gpu_name
    ...
    os.makedirs(config.save_path_stage2, exist_ok=True)
    bestloss = 10000.0
    for epoch in range(1,args.max_epoch+1):
        print('----train-epoch_stage2: '+str(epoch))
        model.train()
        total_loss=0
        ...
        summary.add_scalar('epochloss_in_train_stage2', ave, epoch)
        with open(os.path.join(config.save_path_stage2, "stage2_loss.txt"), "a") as f:
        ...
        if ave<bestloss:
            bestloss =ave
            torch.save(model.state_dict(), os.path.join(config.save_path_stage2, 'model_stage2_best' + '.pth'))
        if 1:
            model.eval()
            with torch.no_grad():
                ...
                #with open(os.path.join(config.save_path_stage2, "stage2_top1_acc.txt"), "a") as f:
                #    f.write(str(total_correct_1 / total_num * 100) + " ")
                #with open(os.path.join(config.save_path_stage2, "stage2_top5_acc.txt"), "a") as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_epoch', default=120, type=int, help='')
    parser.add_argument('--pre_model', default=config.pre_model, type=str, help='')

    args = parser.parse_args()
    logpath = './log_task1_stage2/log--real/'

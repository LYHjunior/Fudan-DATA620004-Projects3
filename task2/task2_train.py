...
def main(cfg):
    if (cfg.DATA.augmentation,cfg.DATA.cutout,cfg.DATA.cutmix,cfg.DATA.mixup)==(True,False,False,False):
        log_name='baseline'
    elif (cfg.DATA.augmentation,cfg.DATA.cutout,cfg.DATA.cutmix,cfg.DATA.mixup)==(True,True,False,False):
        log_name='cutout'
    elif (cfg.DATA.augmentation,cfg.DATA.cutout,cfg.DATA.cutmix,cfg.DATA.mixup)==(True,False,True,False):
        log_name='cutmix'
    elif (cfg.DATA.augmentation,cfg.DATA.cutout,cfg.DATA.cutmix,cfg.DATA.mixup)==(True,False,False,True):
        log_name='mixup'
    else:
        raise Exception("current augmentation setting is not allowed!")
    model_output = Path(cfg.MODEL.saved_path)/log_name
    model_output.mkdir(exist_ok =True)
    writer = SummaryWriter(log_dir='final_homework/task2/logs/'+log_name)
    ...
    for epoch in range(cfg.TRAIN.epochs):
        ...
        for i, (batch_x,batch_y) in enumerate(progress_bar):
            ...
            acc='%.3f' % accuracy)
        test_acc, test_loss = test(test_loader,model,device)
        ...
        if test_acc>best_acc:
            ...
            torch.save(model.state_dict(),  Path(cfg.MODEL.saved_path)/log_name/'best_model.pt')
            best_acc=test_acc
        logger.info(row)

        # record
        writer.add_scalar('train_loss', xentropy_loss_avg, epoch)
